...
RtspDecodeSink* RtspDecodeSink::createNew(UsageEnvironment& env, MediaSubsession& subsession, Frame_Callback callBack, const clientId& clientId, char const* streamId) {
    return new RtspDecodeSink(env, subsession, streamId, callBack, clientId);
}

RtspDecodeSink::RtspDecodeSink(UsageEnvironment& env, MediaSubsession& subsession, char const* streamId, Frame_Callback callBack, const clientId& clientId)
    : MediaSink(env),
...
void RtspDecodeSink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
    ...
    //add start code
    mOnFrameCallback((char*)fReceiveBuffer, frameSize, mClientId);
    /*
